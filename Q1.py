...
def get_MSE_CV(test, weight，λ):
    ...
    for i in range(0, len(Y)):
        ...
        MSE = ((np.linalg.norm(np.mat(XW - Y)) ** 2) / X.shape[0])+λ*np.dot(weight.T,weight)  # ||XW-y||^2/N
    return MSE


def get_MSE(data_train, data_test, λ):
    d2_row = data_train.shape[0]
    Train_Column = data_train.shape[1]
    c2 = data_train[:, :Train_Column - 1]
    r2 = np.ones(d2_row)
    Train_y = data_train[:, Train_Column - 1]
    Test_y = data_test[:, - 1]
    Train_X = np.column_stack((r2, c2))  # train
    # lambda2_axis, mse2_axis = [], []
    a2 = np.dot(Train_X.T, Train_X)  #
    b2 = np.linalg.inv(a2 + λ * np.identity(Train_Column))
    v2 = np.dot(b2, Train_X.T)
    print(v2.shape)
    W = np.dot(v2, Train_y)
    m2 = np.dot(Train_X, W) - Test_y
